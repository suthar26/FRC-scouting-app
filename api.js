...
exports.getMatchSummary = function(matchNumber, response){
...
function sendWhenDone(doneQueries, summary, response, matchNumber){
    if(doneQueries[0] && doneQueries[1]){
        ...
        response.json({'summary' : summary,'matchNumber' : parseInt(matchNumber)});
        console.log('sent pit data');
        return true;
    }
}

exports.getTeam = function(teamNumber, response){
...
function sendTeamData(teamData, response, auto, doneQueries, summary, teamNumber){
    ...
    if(doneQueries[1] && doneQueries[0]){
        console.log(summary);
        response.send({'summary' : summary, 'teamNumber' : teamNumber});
